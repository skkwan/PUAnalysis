...
void makeLTauStack(TString name,TString file,TString dir,int s,TString labelX,TString units = "GeV",bool left=false,TString channel = "",TString json = "Golden",bool log = false,bool dndm=false,bool doRatio = false)
{
    ...
    float xR=0.6; //legend parameters
    ...
    if(channel == "#tau_{#mu}#tau_{h}"||channel == "#mu#tau_{h}") hs->Add(ZEE);
    if(channel == "#tau_{h}#tau_{h}") hs->Add(ZEE);
    hs->Add(ZTT);
    ...
    if (!doRatio) hs->GetYaxis()->SetTitleOffset(1.6);
    ...
    TLegend *l = new TLegend(xR,0.6,xR+0.4,0.85);
